...
options_.posterior_function='posterior_function_demo';

estimation(order=1, datafile=fsdat_simul, nobs=192, loglinear, mh_replic=1000, mh_nblocks=2, mh_jscale=0.8,moments_varendo);

% read out the contents of the cell and put them into ndraws by ncolumns
params=cell2mat(oo_.Posterior_function(:,1));
steady_states=cell2mat(oo_.Posterior_function(:,2));
