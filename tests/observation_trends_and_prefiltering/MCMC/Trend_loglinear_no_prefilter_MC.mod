...
estimation(order=1,datafile='../Exp_AR1_trend_data_with_constant',mh_replic=2000,
    mode_file=Trend_loglinear_no_prefilter_MC_mode,
...
if abs(oo_.PointForecast.Mean.Y_obs(end)- y_forecast_100_periods)>1e-4 || abs(oo_.PointForecast.Mean.P_obs(end)- p_forecast_100_periods)>1e-4
