...
for b=fpar:B
    ...
    if run_smoother && (irun(1) > MAX_nsmoo || b == B),
        ...
        switch Cluster_settings
            case 3
                mhLocalFiles(1).mat={([DirectoryName '/' M_.fname '_smooth' int2str(ifil(1)) '.mat'])};
                fid=fopen([M_.fname '_smooth' int2str(ifil(1)) '.mat']);
                mhLocalData(1).mat=fread(fid);
                fclose(fid);
            otherwise
        ...
        switch Cluster_settings
            case 3
                mhLocalFiles(1).mat={([DirectoryName '/' M_.fname '_update' int2str(ifil(1)) '.mat'])};
                fid=fopen([M_.fname '_update' int2str(ifil(1)) '.mat']);
                mhLocalData(1).mat=fread(fid);
                fclose(fid);
            otherwise
    ...
    if run_smoother && (irun(2) > MAX_ninno || b == B)
        ...
        switch Cluster_settings
            case 3
                mhLocalFiles(1).mat={([DirectoryName '/' M_.fname '_inno' int2str(ifil(2)) '.mat'])};
                fid=fopen([M_.fname '_inno' int2str(ifil(2)) '.mat']);
                mhLocalData(1).mat=fread(fid);
                fclose(fid);
            otherwise
    ...
    if run_smoother && nvn && (irun(3) > MAX_nerro || b == B)
        ...
        switch Cluster_settings
            case 3
                mhLocalFiles(1).mat={([DirectoryName '/' M_.fname '_error' int2str(ifil(3)) '.mat'])};
                fid=fopen([M_.fname '_error' int2str(ifil(3)) '.mat']);
                mhLocalData(1).mat=fread(fid);
                fclose(fid);
            otherwise
    ...
    if run_smoother && naK && (irun(4) > MAX_naK || b == B)
        ...
        switch Cluster_settings
            case 3
                mhLocalFiles(1).mat={([M_.fname '_filter_step_ahead' int2str(ifil(4)) '.mat'])};
                fid=fopen([M_.fname '_filter_step_ahead' int2str(ifil(4)) '.mat']);
                mhLocalData(1).mat=fread(fid);
                fclose(fid);
            otherwise
    ...
    if irun(5) > MAX_nruns || b == B
        ...
        switch Cluster_settings
            case 3
                mhLocalFiles(1).mat={([M_.fname '_param' int2str(ifil(5)) '.mat'])};
                fid=fopen([M_.fname '_param' int2str(ifil(5)) '.mat']);
                mhLocalData(1).mat=fread(fid);
                fclose(fid);
            otherwise
    ...
    if run_smoother && horizon && (irun(6) > MAX_nforc1 || b == B)
        ...
        switch Cluster_settings
            case 3
                mhLocalFiles(1).mat={([M_.fname '_forc_mean' int2str(ifil(6)) '.mat'])};
                fid=fopen([M_.fname '_forc_mean' int2str(ifil(6)) '.mat']);
                mhLocalData(1).mat=fread(fid);
                fclose(fid);
            otherwise
    ...
    if run_smoother && horizon && (irun(7) > MAX_nforc2 ||  b == B)
        ...
        switch Cluster_settings
            case 3
                mhLocalFiles(1).mat={(['/' M_.fname '_forc_point' int2str(ifil(7)) '.mat'])};
                fid=fopen(['/' M_.fname '_forc_point' int2str(ifil(7)) '.mat']);
                mhLocalData(1).mat=fread(fid);
                fclose(fid);
            otherwise
